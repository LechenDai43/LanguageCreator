...
public class PopulateScrollViewPageThree : MonoBehaviour
{
    ...
    public void populate()
    {
        ...
        // Get the vowel phone pool from the manager
        PhoneManager phoneManager = manager.phoneManager;
        VowelPhone[][] vowelPhones = phoneManager.vowelPool;
        ...
        for (int i = 0; i < vowelPhones.Length; i++)
        {
            VowelPhone[] row = vowelPhones[i];
            bool empty = true;
            for (int j = 0; j < row.Length; j++)
            {
                if (vowelPhones[i][j] != null)
                {
                    empty = false;
                    break;
                }
        ...
        for (int j = 0; j < vowelPhones[0].Length; j++)
        {
            bool empty = true;
            for (int i = 0; i < vowelPhones.Length; i++)
            {
        ...
        int validColumnNum = vowelPhones[0].Length - emptyColomn.Count;
        ...
        newItem.transform.GetChild(0).GetComponent<Text>().text = phoneManager.emptyVowel.IPA;
        newItem.transform.GetComponent<PhoneElementItem>().phone = phoneManager.emptyVowel;
        for (int i = 0; i < vowelPhones[0].Length; i++)
        {
